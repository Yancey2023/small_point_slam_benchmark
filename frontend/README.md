...
服务会扫描 `results/<dataset>/<algorithm>`（多 bag 数据集会增加 bag 层级）。重启网页服务后不需要重新运行算法，已有完整 CSV 会直接出现在轨迹和性能对比面板。性能指标从原始 `timings.csv` 动态生成，除总耗时和 CPU 外，还包括每个算法阶段的平均、中位、P95、最大、累计耗时与调用次数。
