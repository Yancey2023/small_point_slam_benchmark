...
服务会扫描 `results/<dataset>-<bag>-<algorithm>`。重启网页服务后不需要重新运行算法，已有 CSV 会直接出现在轨迹、性能和精度对比面板；旧版分层目录仍可读取，新任务统一写入扁平目录。带 ground truth 的轨迹图会叠加真值；精度面板以 SE(3) 刚体对齐后的 ATE RMSE 评价每个算法，无法计算时显示失败原因。性能指标从结果 CSV 动态生成，除算法处理墙钟时间、算法 CPU 总时间、CPU 占用和内存外，还包括各类传感器消息处理，以及通用 LIO 阶段的平均、P95、最大、累计耗时与调用次数。
