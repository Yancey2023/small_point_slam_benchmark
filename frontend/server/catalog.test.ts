...
describe('runtime catalog', () => {
  it('discovers new dataset and algorithm manifests without frontend mappings', async () => {
    ...
    expect(catalog.response.datasets.map((item) => item.bagName)).toEqual([
      'sequence',
      'end_pose_sequence',
    ])
    ...
    expect(catalog.response.datasets[1]).toMatchObject({
      hasGroundTruth: true,
      groundTruthFormat: 'end_pose',
      sensorTypes: ['lidar'],
    })
